...
public class Programa_Main extends JFrame implements ActionListener {
	...
	public void Entrar_jugadores(Object botones) {
		...
		for (int r=0; r<Taldea.size() && Taldea.get(r).getIzena().equals(((JButton)botones).getText()) ;r++) {
	...
	public void Crear_jugadores() {
		...
		for (int o=0;o<Taldea.size() && Encontrado != true ;o++) {
			if(Taldea.get(o).getIzena().equals(Equipo)) {
				Encontrado =true; 
				Taldea.get(o).getJokalaria().add(Jugador);
			}
		}
		Sortu_jokalari.setVisible(false);
		Selekzio_Administrador.setVisible(true);
